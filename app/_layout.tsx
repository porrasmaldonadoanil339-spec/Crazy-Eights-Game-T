...
import { scheduleAllNotifications, requestNotificationPermissions, scheduleChestReadyNotification } from "@/lib/notifications";
import { CHEST_CONFIG } from "@/lib/chestSystem";
...
// "game-online" is included so the AudioManager owns the menu→game transition
// (especially for ranked, where the player jumps straight from matchmaking to
// the match — no menu music should leak into the ranked match).
const GAME_MUSIC_ROUTES = ["game", "game-multi", "game-online"];
...
function AudioManager() {
  ...
  // Task #85 — keep the audio manager in sync with the player's custom intro
  // clip so picking / removing one takes effect immediately for previews and
  // for the next boot.
  useEffect(() => {
    if (!isLoaded) return;
    setCustomStingerUri(profile.customLogoStingerUri || null);
  ...
  useEffect(() => {
    ...
    const sub = AppState.addEventListener("change", (nextState) => {
      if (nextState === "background" || nextState === "inactive") {
        ...
        // Pause (don't stop) so the track resumes from the same position
        // when the user comes back to the app.
        pauseMusic().catch(() => {});
      } else if (nextState === "active") {
        setAppBackgrounded(false).catch(() => {});
        // Try to resume the existing player first; if it was torn down
        // for any reason, fall back to recreating the last requested track.
        resumeMusic()
          .catch(() => {})
          .then(() => resumeCurrentMusic().catch(() => {}));
      }
    });
    return () => sub.remove();
...
function NotificationManager() {
  const { profile, isLoaded, chestInventory } = useProfile();
  const initialized = useRef(false);
  const prevChestCount = useRef(0);

  useEffect(() => {
    if (!isLoaded) return;
    if (initialized.current) return;
    initialized.current = true;
    prevChestCount.current = chestInventory?.length ?? 0;
    if (Platform.OS !== "web") {
  ...
  useEffect(() => {
    if (!isLoaded || !initialized.current) return;
    const currentCount = chestInventory?.length ?? 0;
    if (currentCount > prevChestCount.current && Platform.OS !== "web") {
