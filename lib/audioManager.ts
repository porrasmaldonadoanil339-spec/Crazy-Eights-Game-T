...
const SOUNDS = {
  ...
  // Task #124 — single shared in-game music bed. The previously generated
  // per-mode loops (Practice/Classic/Lightning/Tournament/Challenge/Coop)
  // were swapped out for the calmer Ranked loop because the faster per-mode
  // tracks didn't land. The Ranked loop is now used as the single in-game
  // music for ALL modes, so the unused per-mode requires were removed.
  gameMusicRanked:     require("@/assets/sounds/game-music-ranked.wav"),
  ...
  // Task #119 — premium cinematic stinger that plays under the BIYIS PRIME
  // STUDIOS intro logo on every cold start. Replaces the per-day voice cue
  // with a true AAA-style sonic identity (sub impact + golden chord swell +
  // bell shimmer + soft tail). Generated by scripts/gen-biyis-stinger.mjs.
  biyisStinger:    require("@/assets/sounds/biyis-stinger.wav"),
...
function sourceForTrack(track: MusicTrack) {
  switch (track) {
    case "menu":             return SOUNDS.menuMusic;
    case "search":           return SOUNDS.searchMusic;
    // Task #124 — every in-game track now points to the Ranked bed so the
    // whole game shares the calmer "previous" music regardless of mode.
    case "game_practice":
    case "game_classic":
    case "game_lightning":
    ...
    case "game_coop":
    case "game":
    default:                 return SOUNDS.gameMusicRanked;
  }
}
...
const MODE_TO_MUSIC_TRACK: Record<string, MusicTrack> = {
  ...
  classic:    "game_classic",
  lightning:  "game_lightning",
  challenge:  "game_challenge",
  tournament: "game_tournament",
  ranked:     "game_ranked",
  coop:       "game_coop",
  fichas:     "game_classic",  // shares classic bed; ambience+stinger set it apart
  online:     "game_classic",  // online classic mirrors offline classic
};
...
// Task #124 — end-of-match result stingers. `playVictory` layers the win
// chime with the orchestral fanfare; `playDefeat` plays the dedicated lose
// sting. Both go through `playSfx`, which already short-circuits when the
// SFX toggle is off, so they automatically respect the player's settings
// and never overlap with the menu stinger (which uses its own session
// guard in `playLogoStinger`).
export async function playVictory() {
  await playSfx("win");
  setTimeout(() => { playSfx("victoryFanfare", sfxVolume * 0.95).catch(() => {}); }, 180);
  haptic(async () => {
...
// Task #119 — cinematic BIYIS PRIME STUDIOS intro stinger. Plays once per app
// session under the splash logo. Independent of the menu logo-stinger system
// (which still runs when the main menu mounts). Respects the SFX toggle so
// muting SFX silences the boot identity too.
let biyisStingerPlayedThisSession = false;
export async function playBiyisStinger() {
  if (biyisStingerPlayedThisSession) return;
  biyisStingerPlayedThisSession = true;
  await playSfx("biyisStinger", sfxVolume * 1.0);
}
