...
export function playCard(state: GameState, card: Card, chosenSuit?: Suit): GameState {
  ...
  if (card.rank === "8") {
    ...
    ns.currentSuit = card.suit;
    ns.pendingDraw = 0; ns.pendingDrawType = null; ns.pendingDrawSuit = null;
    ns.message = gm("playerQReverse");
  } else if (card.rank === "K") {
    // K — turno adicional (jugador juega de nuevo)
    ns.currentSuit = card.suit;
    ns.pendingDraw = 0; ns.pendingDrawType = null; ns.pendingDrawSuit = null;
    ns.message = gm("playerKExtra");
  } else {
...
const GM: Record<string, Record<string, string>> = {
  ...
  playerJSkip:    { es:"Jugaste J → Rival pierde su turno. ¡Juegas de nuevo!",en:"You played J → Rival loses their turn. Play again!",pt:"Você jogou J → Rival perde a vez. Jogue de novo!",fr:"Vous avez joué J → Rival perd son tour. Rejouez !",de:"Du spieltest J → Rival verliert seinen Zug. Nochmal spielen!",it:"Hai giocato J → Rivale perde il turno. Gioca ancora!",tr:"J oynadın → Rakip sırasını kaybetti. Tekrar oyna!",ru:"Ты сыграл J → Соперник пропускает ход. Играй снова!",pl:"Zagrałeś J → Rywal traci turę. Graj ponownie!",nl:"Je speelde J → Rival verliest zijn beurt. Speel opnieuw!",sv:"Du spelade J → Rival förlorar sin tur. Spela igen!",da:"Du spillede J → Rival mister sin tur. Spil igen!",fi:"Pelasit J → Vastustaja menettää vuoronsa. Pelaa uudelleen!",no:"Du spilte J → Rival mister turen. Spill igjen!",zh:"你出了J → 对手失去回合，你再出一张！",ja:"Jをプレイ → ライバルがターンをスキップ。もう一度！",ko:"J 플레이 → 상대방이 턴 잃기. 다시 플레이!",hi:"आपने J खेला → प्रतिद्वंद्वी की बारी छूटी। फिर खेलें!",th:"คุณเล่น J → คู่แข่งเสียตาเดิน เล่นอีกครั้ง!",vi:"Bạn đánh J → Đối thủ mất lượt. Chơi lại!",id:"Kamu main J → Rival kehilangan giliran. Main lagi!",ar:"لعبت J → المنافس يخسر دوره. العب مجدداً!" },
  playerQReverse: { es:"Jugaste Q → ¡Dirección invertida! Juegas de nuevo",en:"You played Q → Direction reversed! You play again",pt:"Você jogou Q → Direção invertida! Jogue de novo",fr:"Vous avez joué Q → Direction inversée ! Rejouez",de:"Du spieltest Q → Richtung umgekehrt! Du spielst nochmal",it:"Hai giocato Q → Direzione invertita! Gioca di nuovo",tr:"Q oynadın → Yön tersine döndü! Tekrar oyna",ru:"Ты сыграл Q → Направление изменено! Играй снова",pl:"Zagrałeś Q → Kierunek odwrócony! Graj ponownie",nl:"Je speelde Q → Richting omgekeerd! Speel opnieuw",sv:"Du spelade Q → Riktning omvänd! Spela igen",da:"Du spillede Q → Retning vendt! Spil igen",fi:"Pelasit Q → Suunta kääntyi! Pelaa uudelleen",no:"Du spilte Q → Retning snudd! Spill igjen",zh:"你出了Q → 方向反转！你再出一张",ja:"Qをプレイ → 方向逆転！もう一度プレイ",ko:"Q 플레이 → 방향 역전! 다시 플레이",hi:"आपने Q खेला → दिशा पलटी! फिर खेलें",th:"คุณเล่น Q → ทิศทางกลับ! เล่นอีกครั้ง",vi:"Bạn đánh Q → Đảo chiều! Chơi lại",id:"Kamu main Q → Arah terbalik! Main lagi",ar:"لعبت Q → الاتجاه عكسي! العب مجدداً" },
  playerKExtra:   { es:"Jugaste K → ¡Turno extra! Juegas de nuevo",en:"You played K → Extra turn! You play again",pt:"Você jogou K → Turno extra! Jogue de novo",fr:"Vous avez joué K → Tour supplémentaire ! Rejouez",de:"Du spieltest K → Extra-Zug! Du spielst nochmal",it:"Hai giocato K → Turno extra! Gioca di nuovo",tr:"K oynadın → Ekstra tur! Tekrar oyna",ru:"Ты сыграл K → Доп. ход! Играй снова",pl:"Zagrałeś K → Dodatkowa tura! Graj ponownie",nl:"Je speelde K → Extra beurt! Speel opnieuw",sv:"Du spelade K → Extra tur! Spela igen",da:"Du spillede K → Ekstra tur! Spil igen",fi:"Pelasit K → Lisävuoro! Pelaa uudelleen",no:"Du spilte K → Ekstra tur! Spill igjen",zh:"你出了K → 额外回合！你再出一张",ja:"Kをプレイ → 追加ターン！もう一度プレイ",ko:"K 플레이 → 추가 턴! 다시 플레이",hi:"आपने K खेला → अतिरिक्त बारी! फिर खेलें",th:"คุณเล่น K → ตาเพิ่ม! เล่นอีกครั้ง",vi:"Bạn đánh K → Lượt thêm! Chơi lại",id:"Kamu main K → Giliran ekstra! Main lagi",ar:"لعبت K → دور إضافي! العب مجدداً" },
  ...
  mpRandomShuffle:{ es:"🔀 Palo cambiado al azar: {s}",en:"🔀 Suit randomly changed: {s}",pt:"🔀 Naipe trocado aleatoriamente: {s}",fr:"🔀 Couleur changée au hasard : {s}",de:"🔀 Farbe zufällig geändert: {s}",it:"🔀 Seme cambiato a caso: {s}",tr:"🔀 Sembol rastgele değişti: {s}",ru:"🔀 Масть случайно изменена: {s}",pl:"🔀 Kolor zmieniony losowo: {s}",nl:"🔀 Kleur willekeurig gewijzigd: {s}",sv:"🔀 Färg ändrad slumpmässigt: {s}",da:"🔀 Farve ændret tilfældigt: {s}",fi:"🔀 Väri vaihdettu satunnaisesti: {s}",no:"🔀 Farge endret tilfeldig: {s}",zh:"🔀 花色随机切换: {s}",ja:"🔀 スートがランダムに変更: {s}",ko:"🔀 무늬가 무작위로 변경됨: {s}",hi:"🔀 पत्ता बेतरतीब बदला: {s}",th:"🔀 ตราถูกเปลี่ยนแบบสุ่ม: {s}",vi:"🔀 Chất đổi ngẫu nhiên: {s}",id:"🔀 Jenis diubah acak: {s}",ar:"🔀 تم تغيير اللون عشوائياً: {s}" },
