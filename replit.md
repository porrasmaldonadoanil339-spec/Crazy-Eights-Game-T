...
- **Game Modes**: **Routing rule**: Practice → difficulty picker → local game; Classic/Lightning/Tournament/Challenge → `startGame()` then directly to `/game` (MatchmakingScreen is the ONLY online UI shown — no online-lobby); Coop → `/online-lobby?directSearch=true` → slot-list → `/game?skipMatchmaking=true` (MatchmakingScreen is SKIPPED); Ranked → `/ranked` → `/online-lobby?directSearch=true` → slot-list → `/game?skipMatchmaking=true`. `hasDifficulty: true` ONLY for Practice mode. `ModeInfoModal` play button shows "BUSCAR PARTIDA" + search icon for online modes, "SELECCIONAR DIFICULTAD" for Practice. No CPU/bot references in non-Practice mode info. Coop - player+ally CPU vs 2 rival CPUs with "ALIADO" badge UI. Challenge - HUD with progress bar + random rules from `lib/challengeRules.ts` (10 rule types); ChallengeRulesModal shown on game start. Practice - hint overlay, AI always difficulty 0. Card and table designs applied in game.
