...
**Latest features**: Matchmaking screen (searching → found animation) before each game, different CPU opponent on each retry, per-emote distinct sounds, mute-CPU-emotes toggle in game header, CPU fires random emotes during game, level-up Clash Royale-style overlay with particles, tab navigation sounds on every tab press. DealAnimation shows deck pile + starter card (face-up) after hand flip phase. Inactivity timer bar (20s, gold→orange→red) auto-draws on player's turn. CardPlayEffect (particle burst) fires on card play when an effect is equipped. J card rule: allows same suit OR J/Q/K face card OR wild when jActive (both in single-player and multiplayer engines).
