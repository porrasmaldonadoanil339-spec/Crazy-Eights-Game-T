...
- **Internet Connectivity Guard**: App REQUIRES internet to play — uses `@react-native-community/netinfo@11.4.1` (matched to Expo SDK 54) for real-time monitoring. `context/NetworkContext.tsx` provides `isConnected`/`isChecking`/`recheckNow`. When offline, a beautiful casino-themed `components/OfflineScreen.tsx` appears (multi-language ES/EN/PT, animated WiFi-off icon, retry button, auto-recovers when reconnected). A "Conexión restaurada" green banner slides in when connection is restored mid-session.
- **Critical crash fixes**: Downgraded `expo-audio` from `^55.0.8` (Expo SDK 55 package) → `^1.1.1` (correct for SDK 54). The 55.x version's `ExpoAudio.js` has top-level `AudioModule.AudioPlayer.prototype.replace` that crashes on Expo Go SDK 54's native module (different constructor signature). Also fixed `netinfo` from 12.0.1 → 11.4.1.
- **Previous sessions**: Google/Facebook OAuth ✓ | Global friend requests ✓ | i18n Win/Lose messages ✓ | i18n In-game hints ✓ | Light mode fixes ✓ | i18n Store localizeItem ✓ | Friends chat/typing indicator ✓ | 981 achievements ✓ | 800 BP tiers ✓ | Room codes ✓ | Sound events ✓ | Deal animation ✓ | Friend invite → auto-game ✓
